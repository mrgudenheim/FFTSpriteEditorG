...
# settings vars
# var settings = preload("res://src/Extensions/FFTSpriteEditorG/SavedSettings.gd").new()
@export_file("*.txt") var extension_layout_path: String = "res://src/Extensions/FFTSpriteEditorG/FFTSpriteEditorG.txt"
...
func _exit_tree() -> void:  # Extension is being uninstalled or disabled
	...
	ExtensionsApi.project.current_project.frames_updated.disconnect(set_frame_layer_selectors_options)
	ExtensionsApi.project.current_project.layers_updated.disconnect(set_frame_layer_selectors_options)
...
func initialize():
	...
	isReady = true
	var pixelorama_is_ready: bool = ExtensionsApi.general.get_global().animation_timeline.frame_hbox.get_child_count() >= ExtensionsApi.project.current_project.frames.size() # prevent crash due to null reference when switching projects
...
func save_settings():
	...
	DirAccess.make_dir_recursive_absolute("user://FFTSpriteEditorG")
	ResourceSaver.save(settings, "user://FFTSpriteEditorG/" + settings_file)
...
func load_settings():
	...
	var loaded_settings
	if FileAccess.file_exists("user://FFTSpriteEditorG/" + settings_file):
		loaded_settings = load("user://FFTSpriteEditorG/" + settings_file)
	else:
		print_debug("Trying to load settings that do not exist: " + "user://FFTSpriteEditorG/" + settings_file)
		push_warning("Trying to load settings that do not exist: " + "user://FFTSpriteEditorG/" + settings_file)
		return
...
class CelSelector:
	...
	func _init(frame_selector: OptionButton, layer_selector: OptionButton, main, frame: int = 0, layer: int = 0):
		...
		if not ExtensionsApi.project.current_project.frames_updated.is_connected(update_options):
			ExtensionsApi.project.current_project.frames_updated.connect(update_options)
		
		if not ExtensionsApi.project.current_project.layers_updated.is_connected(update_options):
			ExtensionsApi.project.current_project.layers_updated.connect(update_options)
