...
func load_data():	
	...
	for type in animation_types.keys():
		var path: String = "res://src/Extensions/FFTSpriteEditorG/SeqData/" + type + "_seq.cfg"
		var seq:Seq = Seq.new()
		seq.set_data_from_cfg(path)
		all_seq_data[seq.name_alias] = seq
		
	for type in shape_types.keys():
		var path: String = "res://src/Extensions/FFTSpriteEditorG/FrameData/" + type + "_shp.cfg"
		var shp:Shp = Shp.new()
	...
	var path: String = "res://src/Extensions/FFTSpriteEditorG/FrameData/frame_data_item.txt"
	var shp:Shp = Shp.new()
...
func load_custom_data():
	var directory:String = "user://FFTSpriteEditorG"
